...
public final class CameraManager {
  ...
  public synchronized void openDriver(SurfaceHolder holder) throws IOException {
    ...
    if (!initialized) {
      ...
      int width = configManager.getScreenResolution().x-100;
      int height = (int)(width*0.6);
      setManualFramingRect(width, height);
//      setManualFramingRect((int)(configManager.getScreenResolution().x/2),configManager.getScreenResolution().y/2);
//      setManualFramingRect(configManager.getCameraResolution().x - 100, configManager.getCameraResolution().y-200);
  ...
  public synchronized Rect getFramingRectInPreview() {
    if (framingRectInPreview == null) {
      ...
      //Used For scanning (when barcode is hold in portrait mode)
     /* rect.left = rect.left * cameraResolution.x / screenResolution.x;
      rect.right = rect.right * cameraResolution.x / screenResolution.x;
      rect.top = rect.top * cameraResolution.y / screenResolution.y;
      rect.bottom = rect.bottom * cameraResolution.y / screenResolution.y;*/
      
      //Used For scanning (when barcode is hold in landscape  mode)
